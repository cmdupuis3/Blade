// small_p2.cpp -- GENERATED by gen_small_p2.py.  Do not hand-edit.
//
// The shipped dense-gram jam uses a FIXED tile width R=5 with a scalar
// remainder.  materializeGramForm bakes the extents as literals, so at each p
// the tile fires floor(p/R) times and p mod R cells fall to the reference body.
// The corpus's largest gram extent is 8.  This sweeps R against p to measure
// what the fixed width costs where the corpus actually lives.
//
...
template <> void sweep_p<3>() {
    ...
    printf("p=%-3d BEST=%s %.3fx   shipped R=5 gets %.3fx  (%.0f%% of best)\n\n",
