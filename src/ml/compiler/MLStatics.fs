...
/// stays ML-free.
///
/// `install()` is idempotent, invoked at the top of MLElaborate.expand (the
/// first ML-aware stop of every compilation) so the builtins are visible to
/// every later resolveStatics pass (elaborator, checkModule, Lowering Phase 0).
...
/// Resolve a point-group NAME against the frozen registry. The diagnostic
/// lives HERE, not `PointSpec.pointGroup` (unregistered reaching it is a compiler bug).
...
/// Convert a static value to a point-group spec: (LABEL_NAME, multiplicity)
...
/// (`ml.total_dim(...)`) normalize to this via MLElaborate and the registry
/// is keyed by it, so a bare `total_dim(...)` doesn't resolve -- reachable only via `import ml`. Keep in sync with MLElaborate.sizingNames.
...
let install () =
    if not installed then
        ...
        // CG-typed contraction surface: full decomposition spec of s1 x s2 and
        // the Schur dim of Hom_G -- both pure spec arithmetic, so both are `let static` land.
        ...
        // S2-compacted self-TP weight spaces: exchange-symmetric / antisymmetric
        // halves of tp_full_weight_dim(s, s), the buffer extents of
        // ml.derive_sym_tp / ml.derive_alt_tp. Sum = dense dim
        // (MLSpec.s2TpSplitIsPartition); either half can be 0, making that op a BL4007 at the call site.
        ...
        // Symmetric / exterior powers of a spec: O(3) irrep decomposition of
        // Sym^K(V) / Lambda^K(V) by integer weight-peel, as an ordinary SPEC
        // static (composes with total_dim / hom_dim / irreps_*, writable in
        // IrrepsIdx<>, like tp_spec). K capped at 4: monomial-basis
        ...
        // homogeneous equivariant polynomial map V -> W is dim Hom_G(Sym^K V, W).
        // At K=2, SPEC_OUT=tp_spec(S,S) this IS sym_tp_weight_dim(S) -- two
        // derivations of the same count, pinned against each other in the corpus.
        ...
        // The S_n index-action surface's sizing pair. NO spec argument: a
        // permutation module is named by its RANK and node-axis extent alone
        // -- why S_n is a sibling registry member, not a second block-spec family.
        ...
        // Both ERROR below N = K+L rather than the truncated lattice
        // (no-silent-fork rule), shared with the ops through
        ...
        // Block-navigation builtins: fully static per-block accessors so
        // users write block-structured loop nests --
        //   x(irreps_offset(spec, b) + mu * irreps_dim(spec, b) + m)
        // -- with every offset/bound folding at compile time; pure StaticEval, no codegen involvement.
        ...
        // The POINT-GROUP sizing surface. Every builtin takes GROUP (a static
        // string naming a registered group) first and returns an INT. NO pg
        // builtin returns a SPEC: no pg op yet consumes a derived spec, so
        // those forms (the pg analogue of tp_spec / sym_spec) are deferred.
        //
        // `pg_hom_dim` carries the thesis: the FS formula Sum_i m_i*n_i*e_i
        // reads 9 at C4 (E complex, e=2) and 5 at D4 (E real, e=1) on the
        // SAME spec shape [trivial x 1, E x 2] and R90 generator -- one
        // differing input. `pg_irreps_fs` exposes e itself so a user can SEE the factor.
        ...
        // subgroup inclusion; an int-returning form would answer a question nobody asked.
        //
        // IT IS A SPEC, NOT A CAST: it names the decomposition of D^spec
        // restricted along the group's O(3) embedding. It does NOT say an
        // `IrrepsIdx<SPEC>` buffer may be READ as `PgIrrepsIdx<GROUP,
        // pg_restrict(GROUP, SPEC)>` -- the layouts differ (O(3) orders a
        // block by m=-l..l, so the invariant m=0 sits in the MIDDLE; a pg
        // block is contiguous -- see MLPointSpec's restriction header).
        // Parity is FORGOTTEN: both shipped embeddings are proper rotation
        // groups, so pg_restrict(G,[(0,1,1)]) is the trivial label -- a pseudoscalar becomes a genuine invariant of G.
