...
/// Thin SVD, m >= n: returns (U mxn, S length-n descending, V nxn) with
/// A ~= U*diag(S)*V^T, i.e. A[i,j] = Sigma_k U[i,k]*S[k]*V[j,k].
let svd (sweeps: int) (a: float[,]) : float[,] * float[] * float[,] =
    ...
    // U = normalized columns (zero sigma -> zero column)
...
/// (Q nxn, LAM length-n descending) with S ~= Q*diag(LAM)*Q^T. Same
    ...
    for _sweep in 1 .. sweeps do
        for p in 0 .. n - 2 do
            for r in p + 1 .. n - 1 do
                ...
                let cs = if conv then 1.0 else 1.0 / sqrt (1.0 + tt * tt)
                let sn = if conv then 0.0 else cs * tt
                // A <- R^T A R on the (p, r) plane, columns transform like V
                for i in 0 .. n - 1 do
                    let tp = aw.[i, p]
                    let tq = aw.[i, r]
                    aw.[i, p] <- cs * tp - sn * tq
