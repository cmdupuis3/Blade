...
(* with polynomial reconstructions, read as identities valid in the      *)
(* dual numbers over Z -- what a symbolic certificate provides.  That    *)
(* is narrower than the draft's C^1 statements and is not a substitute   *)
(* for them: a continuous non-polynomial encoding is untouched.  The     *)
(* point chosen for P7 is one point, which is all a lower bound needs;   *)
(* the draft's stronger claim, rank min(N, H) at EVERY point with        *)
(* distinct positive coordinates, is BladeDescartes (P7a and P7b by      *)
(* Descartes' rule, since Rolle is false over Q).  P8 is proved for      *)
(* pure powers, not for the general fragment with coefficient            *)
