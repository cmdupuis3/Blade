(* ===================================================================== *)
(* BladeCauchy.v -- the rank-2 storage split.                            *)
(*                                                                       *)
(* Take any rank-4 tensor T(i1, j1, i2, j2) that is symmetric under      *)
(* exchanging its two (i, j) slots -- the only symmetry a commutative    *)
(* kernel over one repeated array guarantees.  Split it (over Z, with a  *)
(* factor 2 to avoid division) as 2T = Psym + Qalt, where Psym adds and  *)
(* Qalt subtracts the copy of T with its second components swapped.      *)
(* Checked:                                                              *)
(*  - Psym is symmetric under swapping the i-pair and, separately, the   *)
(*    j-pair (the _lat lemmas use the slot symmetry -- that is where     *)
(*    the split earns its keep); Qalt flips sign under each swap and     *)
(*    vanishes whenever either pair coincides.                           *)
(*  - cauchy_split_access: T at ANY index is recovered from the two      *)
(*    component values at the per-dimension-SORTED cell, with a sign     *)
(*    equal to the product of the two sort parities.  So storing each    *)
(*    component in per-dimension triangular form is lossless.            *)
(*  - cauchy_cell_count / cauchy_cells_3_3: the two component stores     *)
(*    together have exactly as many cells as the joint store             *)
(*    (36 + 9 = 45 at extent 3, computed from the live enumerations).    *)
(*                                                                       *)
(* Why it matters: covariance-class tensors (rank 2) recover exact       *)
(* per-dimension storage structure, which naive product storage          *)
(* provably lacks (BladeCounting).  This is the classical Cauchy         *)
(* decomposition Sym^2(V (x) W) = Sym^2 (x) Sym^2 + Wedge^2 (x) Wedge^2  *)
(* made operational; cite Cauchy for the mathematics.                    *)
(*                                                                       *)
(* Scope: rank 2 only -- and the boundary is now a THEOREM, not an open  *)
(* question: BladeDichotomy.v closes r >= 3 negatively (an explicit      *)
(* witness whose two components both vanish; a width-2 refutation over   *)
(* ANY commutative ring) and pins the r! = 6 component repair, so this   *)
(* file's split is optimal, not merely correct.  The total cell count    *)
(* is unchanged -- the gain is                                           *)
(* structure, not size, and each read costs two lookups plus a halving;  *)
(* wiring to the concrete storage layouts is routine and not done here.  *)
(* Naming: the _lat / _lon suffixes mean the first / second index of     *)
(* each slot.                                                            *)
(*                                                                       *)
