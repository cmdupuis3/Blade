...
(*                                                                       *)
(* Scope, stated once.  What is NOT proved is the draft's existence      *)
(* claim for coefficient POLYNOMIALS: that "a_d not identically zero"    *)
(* yields an array of distinct positive reals where a_d(q_r(x)) <> 0.    *)
(* For a concrete a_d that is one evaluation; in general it needs the    *)
(* image of q_r to be Zariski dense (algebraic independence of power     *)
(* sums).  Arrays are listed in decreasing order (lstN); a collision at  *)
(* one ordering refutes G on all arrays of that extent.  Exact real      *)
(* arithmetic.                                                           *)
(*                                                                       *)
