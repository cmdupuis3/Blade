...
| [plan-equivariant-nn-notebooks.md](plan-equivariant-nn-notebooks.md) | ALL THREE NOTEBOOKS BUILT; §5 reverse-AD next | NB1a Tetris deduction (chirality as a compile error), NB1b MD17-aspirin benchmark (full-stack training, live `plot.stream` loss, forces = −∂E/∂r transforming at 1e-15), NB2 pair-cloud moment jets (theorem pins exact; honest outcome: loses to the histogram, correlation ceiling 0.11 — radial windowing is the named follow-on) + the display-stream infra (sink, `emit_id`, `plot.stream`, notebook renderer). 46-entry gap census incl. the ide-serve memo bug (#40) and the interp/codegen reduce divergence (#44) |
