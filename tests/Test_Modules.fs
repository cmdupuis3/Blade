// Test sources live on disk in tests/corpus (audit §2.3 / Phase 0.1: the
// corpus doubles as the differential oracle for the rewrite). This module
// only names the categories; edit the .blade files to change tests.
module Blade.Tests.Modules

open Blade.Tests.Corpus

/// Single-file module tests
let moduleTests = category "modules"

/// Multi-file module tests: each corpus subdirectory is one test, one
/// .blade per module file, compiled together via lowerMultiSource.
let multiFileTests = multiFileCategory "multifile"
