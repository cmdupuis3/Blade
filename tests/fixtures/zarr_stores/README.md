...
- `tests/fixtures/zarr_stores/<store>` (this directory) — resolved at the
  **compiler's** cwd for compile-time metadata loads (`z.load("...")`) and
  static folds.
- `generated_cpp_tests/tests/fixtures/zarr_stores/<store>` — the mirror resolved at
  the **test executable's** cwd for runtime reads; `*_out` stores written by
  the compiled programs land there too.

The same relative path string (`tests/fixtures/zarr_stores/<store>`) is baked into
