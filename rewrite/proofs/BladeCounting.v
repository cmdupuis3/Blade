...
(* equivalently prod_j C(n_j + r - 1, r) < C(prod n_j + r - 1, r)        *)
(* (counting_general_C).  Per-dimension product storage has strictly     *)
(* fewer cells than the joint space has distinct values, so no lossless  *)
(* product layout exists, at any rank and dimension count.               *)
(*                                                                       *)
(* Proof, directly over the enumerations: the pairing e(x, y) = x*b + y  *)
(* sends pairs of sorted tuples to sorted tuples injectively             *)
(* (zipe_canonical, zipe_inj); the tuple 1 :: b :: ... :: b is sorted    *)
(* but its unique decode has a non-sorted second component, so it lies   *)
(* outside the image (witness_not_in_image); pigeonhole                  *)
(* (NoDup_incl_length) gives strict inequality for two factors, and      *)
(* induction on the dimension list lifts to d.                           *)
(*                                                                       *)
(* Classical context: this inequality is the leading-term deficit of    *)
(* the Cauchy decomposition                                              *)
(*   Sym^r(V (x) W) ~= sum over partitions lambda of r of                *)
(*                     S^lambda(V) (x) S^lambda(W).                      *)
(* Product storage captures only the lambda = (r) term.  At r = 2 and    *)
(* extents 3, 3: 45 = 36 + 9, and the nine missing cells are exactly     *)
(* the antisym (x) antisym component.  Cite Cauchy: the contribution     *)
(* here is the elementary kernel-checked form over the enumerations.     *)
(* Per-dimension Reynolds symmetrization is projection onto the leading  *)
(* term, and the constructive r = 2 storage split the decomposition      *)
(* suggests is proved in BladeCauchy.v.                                  *)
(*                                                                       *)
