...
(*                                                                       *)
(* CLASSICAL CONTEXT (recorded v11).  This theorem is the shadow of the  *)
(* Cauchy decomposition from classical invariant theory:                 *)
(*                                                                       *)
(*   Sym^r(V (x) W)  ~=  direct sum over partitions lambda of r of       *)
(*                       S^lambda(V) (x) S^lambda(W)                     *)
(*                                                                       *)
(* (S^lambda the Schur functor).  Product-simplex storage captures       *)
(* exactly the leading lambda = (r) component Sym^r(V) (x) Sym^r(W);     *)
(* the inequality below states that for dim V, dim W >= 2 and r >= 2     *)
(* the decomposition has more than its leading term.  Numerically at     *)
(* r = 2, dims 3 and 3: dim Sym^2(V (x) W) = 45; Sym (x) Sym             *)
(* contributes 36; Lambda^2 (x) Lambda^2 contributes the missing 9 --    *)
(* the nine aliased cells of BladeCore's counterexample ARE the          *)
(* antisym (x) antisym isotypic component.  Consequences:                *)
(*  1. CITE CAUCHY.  This file's contribution is the elementary,         *)
(*     self-contained, kernel-checked form over the enumerations, not    *)
(*     new combinatorics.                                                *)
(*  2. Per-dimension Reynolds symmetrization is projection onto the      *)
(*     leading Cauchy term.                                              *)
(*  3. At r = 2 the decomposition suggests a CONSTRUCTIVE storage        *)
(*     split: a jointly-symmetric tensor splits under per-dim swaps      *)
(*     into sym (x) sym plus antisym (x) antisym parts (character        *)
(*     argument: joint symmetry forces the two dimension characters to   *)
(*     multiply to +1), each losslessly product-storable via SymIdx and  *)
(*     sign-tracked AntisymIdx at exactly the joint total               *)
(*     (36 + 9 = 45).  Dimension- and character-checked only; NOT        *)
(*     mechanized -- see ROADMAP.                                        *)
(* ===================================================================== *)
