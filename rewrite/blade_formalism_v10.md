...
| Fusion | Limited/heuristic | Automatic |         Algebraic combinators
...
```
zip(A, B, C)[i₁]...[iₘ] = Tuple(A[i₁]...[iₘ], B[i₁]...[iₘ], C[i₁]...[iₘ])
```
...
``` blade
// Compound index type constructor
<Idx<n>, Idx<m>>              // compound of two index types

// Type alias for compound indices
type latLonIdx = <Idx<lat>, Idx<lon>>

// In array declarations
Array<Float, latLonIdx>       // equivalent to Array<Float, Idx<lat>, Idx<lon>>
Array<Float, <Idx<n>, Idx<m>>>
...
**Rationale**: Parentheses `(X, Y)` are ambiguous (value tuple or type tuple?). Angle brackets `<...>` are unambiguous—the parser knows `<...>` must contain types. Compound index types as first-class constructs replace runtime meshgrid allocation with compile-time iteration structure.

**In iteration**:
``` blade
// With arrays and explicit compound index
for (A, B) in <Idx<lat>, Idx<lon>> <@> lambda(i, j, a, b) -> ...

// Index-only with type alias
...
```blade
Δ ⊢ A₁ : Array<τ, I>  ...  Δ ⊢ Aᵣ : Array<τ, I>
Δ ⊢ k : (τ^r → τ)
───────────────────────────────────────────────
Δ ⊢ method_for(A₁,...,Aᵣ) <@> k : Comp<τ^r(σ)>
```
...
where
    ...
    omp(xₘ: depth, ...),     // parallelism: depth levels per argument
...
```blade
for range<SymIdx<2, N>> <@> lambda(i, j) -> i * j
```
...
|------|------------------|------------------|
| Indices only | `for range<I> <@> lambda(i,j) -> ...` | `for lambda(i,j) -> ... <@> range<I>` |
...
```blade
Δ ⊢ M : MethodLoop[A₁...Aₙ]    f : arity-polymorphic
compatible(n, f)
c = commutativity(f, n)
...
```blade
Δ ⊢ A₁ : Array<τ, n>  ...  Δ ⊢ Aᵣ : Array<τ, n>
Δ ⊢ k : (τ^r → τ) with comm(...)
─────────────────────────────────────────────────
Δ ⊢ method_for(A₁,...,Aᵣ) <@> k : Comp<Array<τ, n^r, σ>>
```
...
```
Δ ⊢ e : τ        Expression e has type τ in context Δ
Δ ⊢ L : Loop[S]  L is a loop with structure S
Δ ⊢ C : Comp[τ]  C is a computation producing type τ
```
...
```
Δ, x₁:T₁^r₁, ..., xₙ:Tₙ^rₙ ⊢ body : T^r
metadata = (c, p, tdim)
well_formed(metadata)
──────────────────────────────────────────────── (Fun-Intro)
Δ ⊢ (fn(x₁...xₙ) where metadata -> T^r {body; expr}) : Function
```

### 15.4 Loop Object Rules

```blade
Δ ⊢ A₁ : T₁^r₁(σ₁)  ...  Δ ⊢ Aₙ : Tₙ^rₙ(σₙ)
...
```blade
Δ ⊢ M : MethodLoop[S]    Δ ⊢ f : (T₁^r₁...Tₙ^rₙ) → T^r
compatible(S, f)
...
Δ ⊢ O : ObjectLoop[f]    Δ ⊢ A₁ : T₁^r₁(σ₁) ... Δ ⊢ Aₙ : Tₙ^rₙ(σₙ)
compatible(f, A₁...Aₙ)
S = computeStructure(A₁...Aₙ)
...
for lambda(i, j) -> i * j <@> range<SymIdx<2,N>>
```
...
|--------------|---|---|-------|
| **S/T paradigm (§2)** | 9.5 | 9.5 | Structure-first orientation where iteration is primary, operations secondary. No prior system embraces this. Mathematically required for (r!)^d speedup. |
| **S/T ↔︎ T/S duality (§2.6)** | 7.5 | 8.0 | T/S combines two primitives (iteration, indexing) into one construct (2→1). S/T fuses them into one concept with two constructors (1→2). |
| **Loop reification** | 9.0 | 9.0 | Part of **Structural Trinity** (§9.6). Nested loops as composable first-class values. **Prior art**: Polyhedral reify as compiler IR; Halide schedules are directives. |
...
| Contribution | N | I | Notes |
|--------------|---|---|-------|
| **Iteration Object Impossibility (Thm 2.3)** | 9.0 | 9.5 | T/S cannot express first-class iteration objects. T-dimensions are relational, preventing kernel-independent iteration specification. |
...
| **S/T Necessity (Thm 2.10)** | 9.0 | 9.0 | S/T orientation is necessary and sufficient for the Structural Trinity. Two independent lemmas. |
...
| Trees as generalized arrays | 7.5 | 6.5 | Arrays are trees with fixed depth/branching. Path-based indexing, flat storage with precomputed bijection. |
| Symmetric trees | 8.0 | 5.5 | Commutative children analogous to symmetric tensors. Storage reduction via canonical ordering. Genuinely novel but speculative. |
| **Overall (fully realized)** | **9.2** | **9.6** | Petabyte-scale differentiable symmetric tensor computation. End-to-end triangular coherence. |
...
| **S-dimension** | (Spatial/Structural) Dimension arising from iterating over input arrays. For array rank r and kernel input rank m, the outer (r − m) dimensions are S-dimensions. |
...
| **T-dimension** | (Temporal/Trailing) Dimension introduced by kernel output, not derived from iteration. Example: FFT transforms time→frequency. |
