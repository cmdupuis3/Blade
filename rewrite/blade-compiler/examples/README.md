...
| `04_trajectory_sensitivity.blade` | Projectile with drag | unit-checked kinematics (derived `velocity`/`acceleration` units) both scalar and lifted to arrays (broadcasts compose unit signatures elementwise; the `Float<meters>` result annotation is checked, not trusted), `grad()` over a quadrature loop with in-file finite-difference cross-checks, and a gated for-in Euler recurrence — the imperative subset used exactly where it belongs |
